...
__attribute__((constructor)) void regex_compile(){
    ...
    mail_from_regex = pcre_compile("(^MAIL FROM:.*?\\<([\\w\\.]+@[\\w\\.]+)\\>|^MAIL FROM:\\s*([\\w\\.]+@[\\w\\.]+))[$\\s]+", PCRE_CASELESS, &pcreErrorStr, &pcreErrorOffset, NULL);
    rcpt_to_regex = pcre_compile("(^RCPT TO:.*?\\<([\\w\\.]+@[\\w\\.]+)\\>|^RCPT TO:\\s*([\\w\\.]+@[\\w\\.]+))[$\\s]+", PCRE_CASELESS, &pcreErrorStr, &pcreErrorOffset, NULL);
